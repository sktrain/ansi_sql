...
-- via korrelierter Subquery: (meist schlechte Performance)
...
-- alternativ mit nicht-korrelierte Subquery: bessere Performance        
SELECT	e.last_name, e.job_id
FROM	employees e
...
-- seltsamerweise funktioniert hier ANY aber nicht ALL bei Snowflake
...
FROM employees
GROUP BY department_id
HAVING AVG(salary) >= (SELECT max(sub.a) FROM 
                        (SELECT AVG(salary) a
						  FROM employees
