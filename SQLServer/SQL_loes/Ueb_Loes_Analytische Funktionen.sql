...
, SALARY
, SUM(SALARY) OVER (PARTITION BY DEPARTMENT_ID) SUM_SAL
, ROW_NUMBER() OVER (PARTITION BY DEPARTMENT_ID ORDER BY DEPARTMENT_ID) ZEILENUMMER
FROM EMPLOYEES;
...
, RANK() OVER (PARTITION BY DEPARTMENT_ID ORDER BY SALARY DESC) RANK_SALARY_DEPARTMENT_ID
FROM EMPLOYEES
ORDER BY DEPARTMENT_ID;

-- 6. Ergänzen Sie Lösung 5, so dass nur die Top Ten, d.h. die teuersten 
-- 10 Mitarbeiter angezeigt werden.
SELECT TOP(10) DEPARTMENT_ID
, LAST_NAME
, SALARY
, RANK() OVER (ORDER BY SALARY DESC) RANK_SALARY_TOTAL
