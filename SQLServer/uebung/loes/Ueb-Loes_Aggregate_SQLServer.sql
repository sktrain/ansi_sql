...
-- 1. Zeigen Sie das höchste Gehalt, das niedrigste Gehalt, die Summe aller Gehälter 
-- und das Durchschnittsgehalt für alle Mitarbeiter an. 
...
SELECT	ROUND (MAX (salary), 0)  	"Maximum",
		...
		STR(ROUND (AVG(salary), 0),10,2) "Durchschnitt"  
                --round rundet zwar, lässt aber die Stellen stehen
		--,ROUND (AVG(salary), 0)
FROM	employees;


-- 2. Verändern Sie die vorherige Abfrage so, dass die Werte je Jobkennung berechnet
...
-- 5. Zeigen Sie die Differenz zwischen dem niedrigsten und höchsten Gehalt je Abteilung an. 
...
-- Schließen Sie alle Gruppen aus, deren Mindestgehalt 6000 oder weniger beträgt.
...
-- 11. Zeigen Sie für alle Mitarbeiter, deren Managerkennung kleiner 130 ist, Folgendes an:
-- Managerkennung,  Job-Kennung und Gesamtgehalt für jede Jobkennung, sortiert nach der
-- Managerkennung.
...
-- 12. Erweitern Sie Aufgabe 11, so dass zusätzlich angezeigt wird:
...
-- 14. Erweitern Sie die Abfrage aus Aufgabe 13, so dass zusätzlich angezeigt wird:
...
-- 16. Erstellen Sie eine Abfrage, um folgende Angaben für alle Abteilungen anzuzeigen,
...
--  wobei die Ausgabe das Maximal- und das Minimalgehalt nach Abteilung, Tätigkeit
