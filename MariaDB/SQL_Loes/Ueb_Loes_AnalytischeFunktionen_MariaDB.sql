-- Übungen: Analytische Funktionen

-- 1a. Zeigen Sie auf herk�mmliche Weise folgendes je Mitarbeiter an:
...
-- 1b. Lösen Sie die gleiche Aufgabe mittels Partitionierung
...
-- oder mit WINDOW-Klausel

SELECT DEPARTMENT_ID, LAST_NAME, SALARY
, min(salary) OVER w MIN_VALUE
, max(SALARY) OVER w MAX_VALUE
FROM EMPLOYEES
...
-- 5. Erg�nzen Sie die Lösung von Aufgabe 4, so dass zusätzlich das Ranking 
...
-- 13. Listen Sie f�r jede Abteilung die Mitarbeiter (Abteilungsnummer, Name, 
...
FROM
    (SELECT YEAR(HIRE_DATE) YEAR
        ...
        , COUNT(EXTRACT(MONTH FROM HIRE_DATE)) AS AMOUNT
     FROM EMPLOYEES
     GROUP BY YEAR(HIRE_DATE), EXTRACT(MONTH FROM HIRE_DATE)
    ) s;

-- Folgendes funktioniert in MariaDB nicht, da wir Aggregatsfunktionen nicht schachteln können!
-- SELECT EXTRACT(YEAR FROM HIRE_DATE) AS "YEAR"  -- YEAR ist Keyword
-- , EXTRACT(MONTH FROM HIRE_DATE) AS "MONTH"
-- , COUNT(EXTRACT(MONTH FROM HIRE_DATE)) AS AMOUNT
-- , SUM(COUNT(EXTRACT(MONTH FROM HIRE_DATE)))
--      OVER (PARTITION BY EXTRACT(YEAR FROM HIRE_DATE)
--      ORDER BY EXTRACT(MONTH FROM HIRE_DATE)) AS AMOUNTUNTIL
-- FROM EMPLOYEES
