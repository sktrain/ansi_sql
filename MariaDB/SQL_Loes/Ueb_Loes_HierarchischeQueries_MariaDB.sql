...
-- werden.
WITH RECURSIVE rek_topdown( lname, sal, depid, level1, eid  ) AS  -- eid wird für Join benötigt
...
WITH RECURSIVE rek_topdown( lname, sal, depid, level1, eid, path  ) AS  -- eid wird für Join benötigt
    (   SELECT last_name, salary, department_id, 1, employee_id
	            , CAST(last_name AS VARCHAR(20000))		-- CAST ist notwendig
        FROM employees WHERE last_name = 'Mourgos'
      UNION ALL
        SELECT e.last_name, e.salary, e.department_id, r.level1+1, e.employee_id
        , CONCAT (r.path, '/', e.last_name)
        FROM employees e JOIN rek_topdown r ON e.manager_id = r.eid
    )
...
WITH RECURSIVE rek_bottomup( eid, lname, mgrid, path ) AS
    (   SELECT employee_id, last_name, manager_id
	           , CAST(last_name AS VARCHAR(20000))		-- CAST ist notwendig
        FROM employees WHERE last_name = 'Lorentz'
      UNION ALL
        SELECT e.employee_id, e.last_name, e.manager_id
        , CONCAT(e.last_name, '/', r.path)
...
-- Bei MARIADB können Sie zum Datum die Anzahl Tage mittels 
-- "<Datum> + INTERVAL 1 DAY" oder aber mittels der DATE_ADD-Funktion 
-- hinzufügen.
...
-- Variante mit DATE_ADD
WITH RECURSIVE my_dates(dt) AS 
    (
        SELECT DATE '2021-01-01' 
      UNION ALL 
        SELECT  DATE_ADD(dt, INTERVAL 1 DAY) FROM my_dates   
            WHERE dt < DATE '2021-12-31' )
SELECT * FROM my_dates;
...
-- De Haan befinden.
WITH RECURSIVE rek_topdown( eid, lname, mgrid, level1 ) AS  
...
WITH RECURSIVE rek_topdown( lname, mgrid, depid, level1, eid  ) AS  -- eid wird f�r Join benötigt
...
SELECT LPAD(lname, LENGTH(lname)+(level1 * 2) -2, '_') , mgrid, depid, level1
FROM rek_topdown;



-- 6.Zeigen Sie die gesamte Managementhierarchie ohne Indianer topdown mit 
-- Mitarbeiterkennung, Managerkennung, Nachnamen, Ebene an 
-- und zus�tzlich je Manager die Gesamtanzahl ihm unterstellter Mitarbeiter
