...
CREATE TABLE kunde (
	kundennummer VARCHAR(4) PRIMARY KEY,	
	kundenname	VARCHAR(30) NOT NULL,
	-- regulärer Ausdruck: Nummer besteht aus genau vier Ziffern
	CONSTRAINT knummer_check CHECK (REGEXP_LIKE(kundennummer, '^[[:digit:]]{4}$')),
	-- regulärer Ausdruck: Name besteht nur aus (mindestens 2) Buchstaben, der erste groß, Rest klein
	CONSTRAINT kname_check CHECK (REGEXP_LIKE(kundenname, '^[[:upper:]][[:lower:]]{1,}$'))
);


CREATE TABLE bank (
	bankleitzahl INTEGER PRIMARY KEY,
...
CREATE TABLE bankverbindung (
	id_nr BIGINT PRIMARY KEY GENERATED ALWAYS AS IDENTITY,
	kundennummer VARCHAR(4),
