...
-- alternativ per OUTER JOIN 

SELECT COALESCE(e.last_name, '') , d.department_id, COALESCE(d.department_name, '')
...
SELECT	last_name, 'keine Abteilung' as "Abteilung"
	FROM	employees	WHERE	department_id IS NULL		
UNION
SELECT	department_name,  CAST(department_id AS VARCHAR(10))
	FROM departments	WHERE   department_id not in ( SELECT department_id
														FROM employees
														WHERE department_id IS NOT NULL )
ORDER BY "Abteilung"; 
