...
WITH RECURSIVE rek_topdown( lname, sal, depid, level1, eid, path  ) AS  --eid wird für Join benötigt
    (   SELECT last_name, salary, department_id, 1, employee_id
        , CAST(last_name AS text)  -- CAST ist notwendig
        FROM employees WHERE last_name = 'Mourgos'
      UNION ALL
        SELECT e.last_name, e.salary, e.department_id, r.level1+1, e.employee_id
        , r.path || '/' || e.last_name
...
WITH RECURSIVE rek_bottomup( eid, lname, mgrid, path ) AS
    (   SELECT employee_id, last_name, manager_id, CAST(last_name AS text) 
...
-- Bei PostgreSQL können Sie zum Datum die Anzahl Tage mit dem +-Operator 
-- hinzufügen.
...
WITH RECURSIVE rek_topdown( lname, mgrid, depid, level1, eid  ) AS  --eid wird für Join benötigt
...
-- Hinweis: Verwenden Sie die spezifische Erweiterung
-- SEARCH DEPTH FIRST 
WITH RECURSIVE rek_topdown( eid, lname, mgrid, jobid, level1 ) AS  
...
FROM rek_topdown ORDER BY order1;
